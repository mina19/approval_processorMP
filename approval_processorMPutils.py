...
def parseAlert(queue, queueByGraceID, alert, t0, config):
    ...
    if alert_type=='new':
        ...
        if queueByGraceID.has_key(key): ### a throttle already exists
            ...
            item = PipelineThrottle(t0, throttleWin, targetRate, group, pipeline, search=search, requireManualReset=requireManualReset, conf=conf, graceDB_url=client)
        ...
        # interested in iDQ information or other updates
            ...
            if 'comment' in alert['object'].keys():
                ...
                if re.match('minimum glitch-FAP', comment): # looking to see if it's iDQ glitch-FAP information
                    ...
                    saveEventDicts(approval_processorMPfiles)
                elif 'Source-classification probabilities computed from detection pipeline' in comment: # got comment structure from Shaon G.
                    embrightUrl = alert['object']['file']
                    embrightDict = g.get(embrightUrl).read()
                    embrightDict = json.loads(embrightDict)
                    record_em_bright(event_dict.data, embrightDict, logger)
