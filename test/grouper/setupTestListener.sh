...
echo "Checking out branch testingGrouper in approval_processorMP"
cd ${APPROVAL_PROCESSORMP_DIR}
git checkout -b testingGrouper origin/testingGrouper
...
[grouper]
; grouperWin is the time window around the event gpstime of the first ungrouped lvalertMP alert's event GPStime over which we will group triggers together
; decisionWin is the time window we wait before making a selection on which event gets the EM_Selected label to proceed to other checks
grouperWin = 2
decisionWin = 60" > childConfig-approval_processorMPTest.ini
...
cd ${APPROVAL_PROCESSORMP_DIR}/test/grouper
